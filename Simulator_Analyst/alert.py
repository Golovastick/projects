...
def check_anomaly(df, metric, threshold=0.2):
...
def run_alerts(chat=None):
    chat_id = chat or -652068442 # Чат курсов по алертам
    bot = telegram.Bot(token=os.environ.get("REPORT_BOT_TOKEN"))


    # для удобства построения графиков в запрос можно добавить колонки date, hm
    # Вычисляем метрики по ленте новостей
    ...
    # Вычисляем метрики по сервису сообщений
    ...
    # Проверяем метрики ленты новостей
    metrics_feed = ['users_feed', 'views', 'likes', 'ctr']
    for metric in metrics_feed:
        is_alert, current_value, diff = check_anomaly(data_feed, metric) # проверяем метрику на аномальность алгоритмом, описаным внутри функции check_anomaly()
        if is_alert:
            ...
            ax = sns.lineplot( # строим линейный график
                data=data_feed.sort_values(by=['date', 'hm']), # задаем датафрейм для графика
    ...
    # Проверяем метрики сервиса сообщений
    metrics_message = ['users_message', 'messages']
    for metric in metrics_message:
        is_alert, current_value, diff = check_anomaly(data_message, metric) # проверяем метрику на аномальность алгоритмом, описаным внутри функции check_anomaly()
        if is_alert:
            msg = '''Метрика {metric}:\nтекущее значение = {current_value:.2f}\nотклонение от вчера {diff:.2%}'''.format(metric=metric,
                                                                                                                        current_value=current_value,
                                                                                                                        diff=diff)
            ...
            ax = sns.lineplot( # строим линейный график
                data=data_message.sort_values(by=['date', 'hm']), # задаем датафрейм для графика
                x="hm", y=metric, # указываем названия колонок в датафрейме для x и y
                hue="date" # задаем "группировку" на графике, т е хотим чтобы для каждого значения date была своя линия построена
                )
